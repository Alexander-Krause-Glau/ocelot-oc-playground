...
class SpanTranslatorTest {
    ...
    void setUp() throws IOException, RestClientException {
        ...
        final Deserializer<EVSpan> evSpanDeserializer = new SpecificAvroSerde<EVSpan>(schemaRegistryClient).deserializer();
        schemaRegistryClient.register(KafkaConfig.OUT_TOPIC+"-value", EVSpan.SCHEMA$);

        evSpanDeserializer.configure(Map.of(AbstractKafkaAvroSerDeConfig.SCHEMA_REGISTRY_URL_CONFIG, "http://dummy"), false);
        ...
        inputTopic = driver.createInputTopic(KafkaConfig.IN_TOPIC,
            Serdes.ByteArray().serializer(), Serdes.ByteArray().serializer());
        outputTopic = driver.createOutputTopic(KafkaConfig.OUT_TOPIC, Serdes.String().deserializer(), evSpanDeserializer);
    ...
    void testTranslation() throws IOException {
        ...
        String expectedOperationName = "net.explorviz.sampleApplication.database.helper.SQLConnectionHandler.createDatabase";
