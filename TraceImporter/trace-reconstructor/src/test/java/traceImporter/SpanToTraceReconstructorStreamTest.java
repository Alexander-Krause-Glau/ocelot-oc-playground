...
class SpanToTraceReconstructorStreamTest {
    ...
    void setUp() throws IOException, RestClientException {
        ...
        //mockSRC.register("STREAM-AGGREGATE-STATE-STORE-0000000005-repartition-key", EVSpanKey.SCHEMA$);

        Topology topo = (new SpanToTraceReconstructorStream(mockSRC)).getTopology();
        System.out.println(topo.describe().toString());

        final Serializer<EVSpan> evSpanSerializer =
            new SpecificAvroSerde<EVSpan>(mockSRC).serializer();
        ...
        inputTopic = testDriver
            .createInputTopic(KafkaConfig.IN_TOPIC, Serdes.String().serializer(), evSpanSerializer);
        outputTopic = testDriver
            .createOutputTopic(KafkaConfig.OUT_TOPIC, Serdes.String().deserializer(),
                traceDeserializer);
    ...
    void testWindowExceedingTrace() {
        // Push two spans with the same trace id and operation name to the input topic with a gap of 5 seconds
        ...
        EVSpan evSpan1 =
            new EVSpan("1", traceId, startTime, endTime, duration, operationName, 2, "samplehost", "sampleapp");
        EVSpan evSpan2 =
            new EVSpan("2", traceId, startTime, endTime, duration, operationName, 1, "samplehost", "sampleapp");
